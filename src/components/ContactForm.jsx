...
const contactInfo = [
  ...
  },
  {
    icon: 'mail',
    title: 'E-mail',
    details: 'falvarado@www.artecnologia.com.mx / administracion@www.artecnologia.com.mx'
  },
  {
    icon: 'phone',
    title: 'Teléfonos',
...
export default function ContactForm() {
  ...
  useGSAP(() => {
    ...
    animateOnScroll('.contact-info-animate', { x: -40, opacity: 0, stagger: 0.15, ease: 'power3.out' }, '.contact-info-animate');

    // 2. Form panel slide in from right
    animateOnScroll('.contact-form-animate', { x: 40, opacity: 0, scale: 0.98, duration: 0.9, ease: 'power2.out' }, '.contact-form-animate');
  }, { scope: containerRef });
      ...
      <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div className="contact-info-animate">
            <h2 id="contacto-title" className="font-headline-lg text-headline-lg text-on-surface dark:text-white mb-6 tracking-tight font-bold">
              CONTÁCTANOS
            </h2>
            <p className="font-body-lg text-body-lg text-text-muted dark:text-slate-300 mb-10 leading-relaxed">
              Somos una agencia joven con gran experiencia dispuesta a llevar tus ideas al entorno digital. Escríbenos o llámanos para programar una asesoría sin costo.
            </p>
              ...
              {contactInfo.map((info, index) => (
                <div key={index} className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-primary-fixed dark:bg-slate-800 rounded-xl flex items-center justify-center shrink-0 shadow-xs border border-outline-variant/10 dark:border-slate-700/50">
                    <span className="material-symbols-outlined text-primary dark:text-secondary-container select-none" aria-hidden="true">
                      {info.icon}
                    </span>
                  </div>
                  <div>
          ...
          <div className="contact-form-animate bg-slate-50 dark:bg-slate-800/40 p-8 rounded-3xl border border-outline-variant/10 dark:border-slate-700/50 shadow-[0px_10px_30px_rgba(0,0,0,0.02)]">
            ...
            <form onSubmit={handleSubmit} className="space-y-4" aria-describedby="contact-form-help">
              <p id="contact-form-help" className="sr-only">
                Los campos nombre, correo electrónico y mensaje son obligatorios.
              </p>
              {/* Name */}
              <div>
                <label htmlFor="nombre" className="block font-bold text-xs text-on-surface-variant dark:text-slate-300 mb-1.5">
                  Nombre (requerido)
                </label>
                <input 
                  type="text" 
                ...
                />
                <span id="nombre-hint" className="block text-[10px] text-text-muted dark:text-slate-400 mt-1">Mínimo 2 caracteres.</span>
                {errors.nombre && <span id="nombre-error" role="alert" className="text-[10px] font-bold text-red-500 mt-1 block">{errors.nombre}</span>}
              </div>

              {/* Email */}
              <div>
                <label htmlFor="correo" className="block font-bold text-xs text-on-surface-variant dark:text-slate-300 mb-1.5">
                ...
                />
                <span id="correo-hint" className="block text-[10px] text-text-muted dark:text-slate-400 mt-1">Formato: nombre@dominio.com</span>
                {errors.correo && <span id="correo-error" role="alert" className="text-[10px] font-bold text-red-500 mt-1 block">{errors.correo}</span>}
              </div>

              {/* Phone & Subject */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label htmlFor="telefono" className="block font-bold text-xs text-on-surface-variant dark:text-slate-300 mb-1.5">
                    Teléfono
                  </label>
                  <input 
                    type="tel" 
                ...
                <div>
                  <label htmlFor="asunto" className="block font-bold text-xs text-on-surface-variant dark:text-slate-300 mb-1.5">
                    Asunto
                  </label>
                  <input 
                    type="text" 
              ...
              {/* Message */}
              <div>
                <label htmlFor="mensaje" className="block font-bold text-xs text-on-surface-variant dark:text-slate-300 mb-1.5">
                  Mensaje (requerido)
                </label>
                <textarea 
                  name="mensaje"
                ...
                />
                <span id="mensaje-hint" className="block text-[10px] text-text-muted dark:text-slate-400 mt-1">Incluye al menos 10 caracteres para entender mejor tu proyecto.</span>
                {errors.mensaje && <span id="mensaje-error" role="alert" className="text-[10px] font-bold text-red-500 mt-1 block">{errors.mensaje}</span>}
              </div>
              ...
              >
                {isSubmitting ? (
                  <>
                    <svg className="animate-spin h-5 w-5 text-white dark:text-slate-900" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true">
        ...
        >
          <div className="relative glass-card text-on-surface dark:text-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl z-50 animate-fade-in-up">
            <div className="w-16 h-16 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center mx-auto mb-6">
              <span className="material-symbols-outlined text-emerald-600 dark:text-emerald-400 text-4xl select-none font-bold" aria-hidden="true">
                done
              </span>
            </div>
            
            <h3 id="success-title" className="font-bold text-2xl tracking-tight mb-2">¡Mensaje Enviado!</h3>
            <p id="success-message" className="text-sm text-text-muted dark:text-slate-300 leading-relaxed mb-6">
              Agradecemos tu interés. Uno de nuestros asesores se pondrá en contacto contigo a la brevedad posible.
            </p>
